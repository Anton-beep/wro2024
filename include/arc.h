...
void reactiveTurnRight() {
    ...
    bool flag = true;
    while (-nMotorEncoder[motorA] + MTVarsA.targetEnc < 200 || CDSensor2.color != 1){
        motorAstop = false;
        motorBstop = false;

        getCDValues(&CDSensor2);
        if (flag && nPgmTime - tt >= 165) {
            flag = false;
...
void reactiveTurnLeft() {
    ...
    unsigned long tt = nPgmTime;
    bool flag = true;
    while (nMotorEncoder[motorB] - MTVarsB.targetEnc < 200 || CDSensor1.color != 1){
        motorAstop = false;
        motorBstop = false;
